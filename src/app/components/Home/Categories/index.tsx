...
const Categories = () => {
  ...
  const getListCategoryData = async () => {
    const data = await getCategory().categoryControllerGetAll();
      ...
      <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0 pb-15 border-b border-gray-3">
        ...
        <div>
          {categoryData.length === 0 ? (
            ...
            <div className="grid grid-cols-4 gap-4 flex-wrap items-center justify-between">
              {categoryData.map((item, idx) => (
                <Link
                  href={`/category/${item.id}`}
                  key={idx}
                  className="bg-blue-100 py-8 text-center cursor-pointer rounded-md group"
                >
